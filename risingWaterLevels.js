 ...
 */
 ...
 // 1) create variable to track lowest water level & largest rise in water level
 // 2) create loop to iterate array 
 // 3) if 
 function measureWaterLevels(waterLevels) {
   let lowest = waterLevels[0];
   let rise = 0;
   for(j=0; j <waterLevels.length;j++){
     if(waterLevels[j] - lowest > rise){ 
       rise = waterLevels[j] - lowest;
     }else if(waterLevels[j] < lowest){
       lowest = waterLevels[j];
     }
   }
   return rise;
 }
